...
void CoeffToSlot_adaptive(const R_Q_square<  LOGQ, 1 << LOGN>& ct,
	const int s[1 << LOGN],
	...
	grouped_serial_linear_transform_adaptive<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G>(U0r, U0i, ct, s, U0ct);
...
template<int LOGQ, int LOGN, int LOGDELTA, int G = 1>
void CoeffToSlot_sw_adaptive(const R_Q_square<  LOGQ, 1 << LOGN>& ct,
	const int s[1 << LOGN],
	R_Q_square<  LOGQ - LOGDELTA, 1 << LOGN> ct_[2]) {
	const int N = 1 << LOGN;
	static bool is_init = false;
	static SparseDiagonal<N / 2, 3> U0r[LOGN - 1];
	static SparseDiagonal<N / 2, 3> U0i[LOGN - 1];
	...
	R_Q_square<LOGQ - LOGDELTA, N> U0ct;
	grouped_serial_linear_transform_sw_adaptive<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G>(U0r, U0i, ct, s, U0ct);

	R_Q_square<LOGQ - LOGDELTA, N> U0ct_conj;
	int s_conj[N];
	R_Q_square<2 * (LOGQ - LOGDELTA), 1 << LOGN> ckey;
	conj<N>(s, s_conj);
	HEAAN<LOGQ - LOGDELTA, N>::swkgen(s_conj, s, ckey);
	conj(U0ct, ckey, U0ct_conj);

	R_Q<LOGQ - LOGDELTA, N> pti; pti.setzero();
	pti[N / 2][0] = 1; pti[N / 2].negate();
	ct_[0] = U0ct; ct_[0] += U0ct_conj;
	ct_[1] = U0ct; ct_[1] -= U0ct_conj; ct_[1] *= pti;
}
...
template<int LOGQ, int LOGN, int LOGDELTA, int G = 1, int LOG_qdiv>
void tuple_CoeffToSlot(const R_Q_square<  LOGQ, 1 << LOGN>& ct0_up, const R_Q_square<  LOGQ, 1 << LOGN>& ct0_down,
	const int s[1 << LOGN],
	R_Q_square<  LOGQ, 1 << LOGN> ct_[2]) {
	const int N = 1 << LOGN;
	static bool is_init = false;
	...
	R_Q_square<LOGQ, N> temp_up, temp_down, U0ct;
	tuple_grouped_serial_linear_transform<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G, LOG_qdiv>(U0r, U0i, ct0_up, ct0_down, s, temp_up, temp_down);
	recomb< LOGQ, N, LOG_qdiv>(temp_up, temp_down, U0ct);
...
template<int LOGQ, int LOGN, int LOGDELTA, int G>
void SlotToCoeff_adaptive(const R_Q_square<  LOGQ, 1 << LOGN>& ct0,
	const R_Q_square<  LOGQ, 1 << LOGN>& ct1,
	const int skey[1 << LOGN],
	R_Q_square<  LOGQ, 1 << LOGN>& ct_) {
	const int N = 1 << LOGN;
	...
	grouped_serial_linear_transform_adaptive<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G>(U0r, U0i, ct0, skey, ct_);
	R_Q_square<LOGQ, N> ct2;
	grouped_serial_linear_transform_adaptive<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G>(iU0r, iU0i, ct1, skey, ct2);
	ct_ += ct2;
}
...
template<int LOGQ, int LOGN, int LOGDELTA, int G, int LOG_qdiv>
void tuple_SlotToCoeff(const R_Q_square<  LOGQ, 1 << LOGN>& ct0_up, const R_Q_square<  LOGQ, 1 << LOGN>& ct0_down,
	const R_Q_square<  LOGQ, 1 << LOGN>& ct1_up, const R_Q_square<  LOGQ, 1 << LOGN>& ct1_down,
	const int skey[1 << LOGN],
	R_Q_square<  LOGQ, 1 << LOGN>& ct_up, R_Q_square<  LOGQ, 1 << LOGN>& ct_down) {
	const int N = 1 << LOGN;
	static bool is_init = false;
	static SparseDiagonal<N / 2, 3> U0r[LOGN - 1];
	static SparseDiagonal<N / 2, 3> U0i[LOGN - 1];
	...
	tuple_grouped_serial_linear_transform<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G, LOG_qdiv>(U0r, U0i, ct0_up, ct0_down, skey, ct_up, ct_down);
	R_Q_square<LOGQ, N> ct2_up, ct2_down, temp_up, temp_down;
	tuple_grouped_serial_linear_transform<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G, LOG_qdiv>(iU0r, iU0i, ct1_up, ct1_down, skey, ct2_up, ct2_down);
	temp_up = ct_up;
	temp_down = ct_down;
	tuple_add(ct2_up, ct2_down, temp_up, temp_down, ct_up, ct_down);
}
...
template<int LOGQ, int LOGN, int LOGDELTA, int G, int LOG_qdiv>
void tuple_SlotToCoeff_adaptive(const R_Q_square<  LOGQ, 1 << LOGN>& ct0_up, const R_Q_square<  LOGQ, 1 << LOGN>& ct0_down,
	const R_Q_square<  LOGQ, 1 << LOGN>& ct1_up, const R_Q_square<  LOGQ, 1 << LOGN>& ct1_down,
	const int skey[1 << LOGN],
	R_Q_square<  LOGQ, 1 << LOGN>& ct_up, R_Q_square<  LOGQ, 1 << LOGN>& ct_down) {
	const int N = 1 << LOGN;
	...
	tuple_grouped_serial_linear_transform_adaptive<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G, LOG_qdiv>(U0r, U0i, ct0_up, ct0_down, skey, ct_up, ct_down);
	R_Q_square<LOGQ, N> ct2_up, ct2_down, temp_up, temp_down;
	tuple_grouped_serial_linear_transform_adaptive<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G, LOG_qdiv>(iU0r, iU0i, ct1_up, ct1_down, skey, ct2_up, ct2_down);
	temp_up = ct_up;
	temp_down = ct_down;
	tuple_add(ct2_up, ct2_down, temp_up, temp_down, ct_up, ct_down);
}
...
template<int LOGQ, int LOGN, int LOGDELTA, int G = 1>
void CoeffToSlot_sw(const R_Q_square<  LOGQ, 1 << LOGN>& ct,
	const int s[1 << LOGN],
	R_Q_square<  LOGQ - LOGDELTA, 1 << LOGN> ct_[2]) {
	const int N = 1 << LOGN;
	static bool is_init = false;
	...
	R_Q_square<LOGQ - LOGDELTA, N> U0ct;
	grouped_serial_linear_transform_sw<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G>(U0r, U0i, ct, s, U0ct);
...
template<int LOGQ, int LOGN, int LOGDELTA, int G = 1, int LOG_qdiv, int LOG_ql>
void tuple_CoeffToSlot_sw(const R_Q_square<  LOGQ, 1 << LOGN>& ct0_up, const R_Q_square<  LOGQ, 1 << LOGN>& ct0_down,
	const int s[1 << LOGN],
	R_Q_square<  LOGQ - LOG_ql, 1 << LOGN> ct_[2]) {
	const int N = 1 << LOGN;
	static bool is_init = false;
	static SparseDiagonal<N / 2, 3> U0r[LOGN - 1];
	static SparseDiagonal<N / 2, 3> U0i[LOGN - 1];
	...
	R_Q_square<LOGQ - LOG_ql, N> temp_up, temp_down, U0ct;
	tuple_grouped_serial_linear_transform_sw<LOGQ, LOGN, LOGDELTA, 3, LOGN - 1, G, LOG_qdiv, LOG_ql>(U0r, U0i, ct0_up, ct0_down, s, temp_up, temp_down);
	recomb< LOGQ - LOG_ql, N, LOG_qdiv>(temp_up, temp_down, U0ct);

	R_Q_square<LOGQ - LOG_ql, N> U0ct_conj;
	int s_conj[N];
	R_Q_square<2 * (LOGQ - LOG_ql), 1 << LOGN> ckey;
	conj<N>(s, s_conj);
	HEAAN<LOGQ - LOG_ql, N>::swkgen(s_conj, s, ckey);
	conj(U0ct, ckey, U0ct_conj);

	R_Q<LOGQ - LOG_ql, N> pti; pti.setzero();
	pti[N / 2][0] = 1; pti[N / 2].negate();
	ct_[0] = U0ct; ct_[0] += U0ct_conj;
	ct_[1] = U0ct; ct_[1] -= U0ct_conj; ct_[1] *= pti;
}
